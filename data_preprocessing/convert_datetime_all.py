"""convert_datetime_all.py converts vid_real_time files to a more appropriate format for indexing
final table is of this form
------------------------------
|patient|session|vid|datetime|
|  ...  |  ...  |...|  ...   |
Usage:  python convert_datetime_all.py /data1/sharedata/vid_real_time/ ../temp_data/

It processes all files in vid_real_time and generates an output table for each patient id in temp_data folder.
...
for filename in filenames[0:]:
    ...
    print(new.head())
    output = sys.argv[2]+'datetime_'+patient+'.csv'
