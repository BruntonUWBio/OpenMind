    ...
    for filename in filenames:
        ...
        if len(events) > 0:
            ...
            epochs.rename_channels(
                {ch_name: i for ch_name, i in zip(epochs.ch_names, ch_names)})

            dig_ch_pos = dict(zip(ch_names, elec))
            mon = mne.channels.DigMontage(
                dig_ch_pos=dig_ch_pos, point_names=ch_names)
